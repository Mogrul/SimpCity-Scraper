...
class Site:
    ...
    def download_file(
        ...
        base_parent, file_path = self.build_file_path(url, additional_references)
        reply = self.session.download_file(url, file_path)
        
        if not reply:
            return None
    
        url, destination = reply
    
        return (url, destination, base_parent)
    ...
    def build_file_path(
        ...
        base_parent = file_path.parent
        
        if additional_references:
            if References.DATE in additional_references:
                file_parent = base_parent
